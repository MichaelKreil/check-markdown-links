...
describe('run on test_data', () => {
	test('their should be no errors', async () => {
		...
		expect(errors[0].toString()).toBe(
			'Unknown link found "sub/doc2.md#header-1" in doc1.md (L:8), sub/doc2.md (L:4,6)'
		);
		expect(errors[1].toString()).toBe('Unknown link found "doc1.md#header-3" in doc1.md (L:9), sub/doc2.md (L:5)');
		expect(errors[2].toString()).toBe(
			'External link unreachable "https://this-domain-would-only-be-bought-by-someone-who-is-bored-and.rich" in doc1.md (L:12)'
		);
	});
});
